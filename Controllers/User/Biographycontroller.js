...
// get all biographies while supporting pagination 6 biographies per page
const GetAllBiographies = async (req, res) => {
    try {
        ...
        const biographies = await Biography.find({ listed: true }).sort({ createdAt: -1 }).limit(limit).skip(startIndex);
        const total = await Biography.countDocuments();
        return res.status(200).json({ biographies, total });
    } catch (error) {
        return res.status(500).json({ message: error.message });
    }
};
